...
public final class KnightLibLivingModel<T extends LivingEntity & KnightLibAnimatable> extends EntityModel<T> implements ArmedModel, HeadedModel {
    ...
    @Override
    public void renderToBuffer(PoseStack poseStack, VertexConsumer consumer, int packedLight, int packedOverlay, int renderColor) {
        if (activeModel != null) {
            dispatchKeyframeEvents(poseStack);
            final KnightLibColor color = KnightLibColor.fromArgb(activeRenderColor).multiply(KnightLibColor.fromArgb(renderColor));
            activeModel.renderLiving(poseStack, consumer, packedLight, packedOverlay, color.red(), color.green(), color.blue(), color.alpha());
        }
