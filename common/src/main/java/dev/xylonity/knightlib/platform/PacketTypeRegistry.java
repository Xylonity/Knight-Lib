...
final class PacketTypeRegistry {
    ...
    synchronized <T> void register(Direction direction, PacketType<T> type) {
        ...
        final Entry<?> previousType = entriesById(direction).get(packetId);
        if (previousType != null) {
            throw new IllegalStateException("[KnightLib] " + direction.name + " packet id " + packetId + " is already registered for " + previousType.type.clazz().getName());
        }

        final Entry<?> previousClass = entriesByClass(direction).get(packetClass);
        if (previousClass != null) {
            throw new IllegalStateException("[KnightLib] " + direction.name + " packet class " + packetClass.getName() + " is already registered as " + previousClass.type.id());
        }
    ...
    void validate(Direction direction, PacketType<?> type, Object message) {
        ...
        if (entry.type.clazz() != type.clazz()) {
            throw new IllegalArgumentException("[KnightLib] Packet " + type.id() + " is registered for " + entry.type.clazz().getName() + " instead of " + type.clazz().getName());
        }
    ...
    private record Entry<T>(
        ...
        private void validateMessage(Object message) {
            ...
            if (!type.clazz().isInstance(message)) {
                throw new IllegalArgumentException("[KnightLib] Packet " + type.id() + " expects " + type.clazz().getName() + " but received " + message.getClass().getName());
            }
        ...
        private Object decode(FriendlyByteBuf buffer) {
            ...
            if (message == null) {
                throw new IllegalStateException("[KnightLib] Packet codec for " + type.id() + " decoded a null message");
            }
            if (!type.clazz().isInstance(message)) {
                throw new IllegalStateException("[KnightLib] Packet codec for " + type.id() + " decoded " + message.getClass().getName() + " instead of " + type.clazz().getName());
            }
