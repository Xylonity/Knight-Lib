...
public record ChaliceFillingRecipe(ItemStack input, ItemStack block) implements Recipe<GenericRecipeInput> {
    ...
    @Override
    public boolean matches(GenericRecipeInput chaliceFillingRecipeInput, @NotNull Level level) {
        return ItemStack.isSameItem(chaliceFillingRecipeInput.getItem(0), input);
    }

    @Override
    public @NotNull ItemStack assemble(@NotNull GenericRecipeInput chaliceFillingRecipeInput, HolderLookup.Provider provider) {
    ...
    public static class Serializer implements RecipeSerializer<ChaliceFillingRecipe> {
        public static final MapCodec<ChaliceFillingRecipe> CODEC = RecordCodecBuilder.mapCodec(
                i -> i.group(
                        ItemStack.CODEC.fieldOf("ingredient").forGetter(ChaliceFillingRecipe::input),
